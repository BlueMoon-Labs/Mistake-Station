...
/datum/round_event/crystalline_wave/announce(fake)
	priority_announce("Several crystalline asteroids have been detected en route with the station. All hands, brace for impact. Organic signals have been detected contained within some of the asteroids.", title = "Priority Alert", sound = 'modular_skyrat/modules/alerts/sound/misc/voyalert.ogg')
...
/obj/effect/crystalline_reentry/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change = TRUE)
...
/obj/effect/crystalline_reentry/Bump(atom/clong)
	...
	if(isturf(clong) || isobj(clong))
		...
		R.create_foam(/datum/effect_system/fluid_spread/smoke, 50)
		R.add_reagent(/datum/gas/water_vapor, 15)
		qdel(src)
		qdel(other)
	if(asteroidhealth <= 0)
		collision_effect()
...
/obj/effect/crystalline_reentry/proc/collision_effect()
	...
	switch(rand(1,100))
		...
		if(100)
			var/mob/living/simple_animal/bot/medbot/catmedbot/M = new(src.loc)
