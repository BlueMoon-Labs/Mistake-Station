...
/mob/living/simple_animal/hostile/deathclaw/funclaw/AttackingTarget()
	...
	if(M.client && M.client?.prefs.erppref == "Yes" && CHECK_BITFIELD(M.client?.prefs.toggles, VERB_CONSENT) && M.client?.prefs.noncon_pref == "Yes")
		wantsNoncon = TRUE
	...
	switch(chosen_hole)
		...
		if(CUM_TARGET_VAGINA)
			if(M.is_bottomless() && M.has_vagina())
				...
				M.impregnate(src, M.get_organ_slot(ORGAN_SLOT_WOMB), src.type)
			else
