...
/datum/component/organ_inflation/proc/inflate_organ(new_size)
	...
	switch(container.slot)
		if(ORGAN_SLOT_PENIS)
			var/obj/item/organ/external/genital/penis/peenus = container.owner.get_organ_slot(ORGAN_SLOT_PENIS)
			peenus?.modify_size(new_size)
		if(ORGAN_SLOT_BREASTS)
			var/obj/item/organ/external/genital/breasts/breasts = container.owner.get_organ_slot(ORGAN_SLOT_BREASTS)
			breasts?.modify_size(new_size)
		else
			var/obj/item/organ/external/genital/belly/belly = container.owner.get_organ_slot(ORGAN_SLOT_BELLY)
			if(!belly && ishuman(container.owner))
				var/mob/living/carbon/human/human_owner = container.owner
				belly = human_owner.give_genital(/obj/item/organ/external/genital/belly)
