...
/obj/item/clothing/sextoy/vibrator/attack(mob/living/carbon/human/target, mob/living/carbon/human/user)
	...
	switch(user.zone_selected) //to let code know what part of body we gonna vibe
		if(BODY_ZONE_PRECISE_GROIN)
			...
			if((vagina && penis) && (vagina.visibility_preference == GENITAL_THROUGH_CLOTHES && penis.visibility_preference == GENITAL_THROUGH_CLOTHES || target.is_bottomless()))
			...
			else if(vagina && (vagina.visibility_preference == GENITAL_THROUGH_CLOTHES || target.is_bottomless()))
			...
			else if(penis && (penis.visibility_preference == GENITAL_THROUGH_CLOTHES || target.is_bottomless()))
