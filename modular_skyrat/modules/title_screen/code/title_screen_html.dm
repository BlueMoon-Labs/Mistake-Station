...
/mob/dead/new_player/proc/get_title_html()
	...
	if(SSticker.current_state == GAME_STATE_STARTUP)
		...
		if(!SSticker || SSticker.current_state <= GAME_STATE_PREGAME)
			dat += {"<a id="ready" class="menu_button" href='?src=[text_ref(src)];toggle_ready=1'>[ready == PLAYER_READY_TO_PLAY ? "<span class='checked'>☑</span> ГОТОВ" : "<span class='unchecked'>☒</span> НЕ ГОТОВ"]</a>"}
		else
			dat += {"
				<a class="menu_button" href='?src=[text_ref(src)];late_join=1'>ВОЙТИ В ИГРУ</a>
		...
		<script language="JavaScript">
			...
			var ready_marks = \[ "<span class='unchecked'>☒</span> НЕ ГОТОВ", "<span class='checked'>☑</span> ГОТОВ" \];
			...
			var antag_marks = \[ "<span class='unchecked'>☒</span> НЕ БЫТЬ АНТАГОНИСТОМ", "<span class='checked'>☑</span> БЫТЬ АНТАГОНИСТОМ" \];
