...
/obj/machinery/jukebox
	...
	verb_say = "states"
	density = TRUE
	req_one_access = list(ACCESS_BAR, ACCESS_KITCHEN, ACCESS_HYDROPONICS, ACCESS_COMMAND, ACCESS_THEATRE)
...
/obj/machinery/jukebox/disco
	...
	icon_state = "disco"
	req_one_access = list(ACCESS_COMMAND, ACCESS_BAR, ACCESS_KITCHEN)
	anchored = FALSE
		...
		if("add_to_queue")
			...
			if(!allowed(usr) && queuecost)
				...
				if(!C)
					...
					playsound(src, 'sound/misc/compiler-failure.ogg', 25, TRUE)
					return
				if(!check_nap_violations(C, queuecost))
...
/obj/machinery/jukebox/disco/proc/dance_setup()
	...
	FOR_DVIEW(var/turf/t, 3, get_turf(src),INVISIBILITY_LIGHTING)
		if(t.x == cen.x && t.y > cen.y)
			...
			L.light_color = COLOR_MOSTLY_PURE_RED
		...
		if((t.x+1 == cen.x && t.y+1 == cen.y) || (t.x+2==cen.x && t.y+2 == cen.y))
			...
			L.light_power = 30-(get_dist(src,L)*8)
			L.base_light_range = 1.4+get_dist(src, L)
			spotlights+=L
			continue
		if((t.x-1 == cen.x && t.y-1 == cen.y) || (t.x-2==cen.x && t.y-2 == cen.y))
			var/obj/item/flashlight/spotlight/L = new /obj/item/flashlight/spotlight(t)
	...
	while(active)
		for(var/obj/item/flashlight/spotlight/glow in spotlights) // The multiples reflects custom adjustments to each colors after dozens of tests
			if(QDELETED(src) || !active || QDELETED(glow))
				return
			if(glow.light_color == COLOR_MOSTLY_PURE_RED)
			...
			if(glow.light_color == LIGHT_COLOR_BLUE)
				...
				glow.light_range = glow.base_light_range * DISCO_INFENO_RANGE
			...
			if(glow.light_color == LIGHT_COLOR_CYAN)
				glow.light_color = COLOR_MOSTLY_PURE_RED
...
/obj/machinery/jukebox/process(mob/user)
	if(active)
		if(world.time >= stop)
			active = FALSE
			dance_over()
			...
			for(var/mob/living/carbon/C in hearers(round(volume/35), src)) // I WANT YOU IN MY ROOM
				if(istype(C)) // LETS SPEND THE NIGHT TOGETHER
					C.adjustEarDamage(max((((volume/35) - sqrt(get_dist(C, src) * 2)) - C.get_ear_protection())*0.1, 0)) // FROM NOW UNTIL FOREVER
