...
/mob/living/proc/copy_to_prefs_vr()
	...
	for(var/belly in vore_organs)
		...
		serialized += list(B.pserialize()) //Can't add a list as an object to another list in Byond. Thanks.
...
/mob/living/proc/lick()
	...
	if(QDELETED(tasted) || (tasted.ckey && !(tasted.client?.prefs.vore_flags & LICKABLE)) || !Adjacent(tasted) || incapacitated(HAS_TRAIT(src, TRAIT_RESTRAINED)))
		return
...
/mob/living/proc/smell_someone()
	...
	if(incapacitated(HAS_TRAIT(src, TRAIT_INCAPACITATED)))
		to_chat(src, "<span class='warning'>You can't do that while incapacitated.</span>")
		return
	if(!CheckActionCooldown())
		to_chat(src, "<span class='warning'>You can't do that so fast, slow down.</span>")
	...
	if(QDELETED(sniffed) || (sniffed.ckey && !(sniffed.client?.prefs.vore_flags & SMELLABLE)) || !Adjacent(sniffed) || incapacitated(HAS_TRAIT(src, TRAIT_RESTRAINED)))
		return
