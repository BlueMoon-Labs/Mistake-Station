		...
		if(M && isbelly(M.loc))
			if(world.time > M.next_preyloop)
				if(is_wet && wet_loop)
					...
					if(M.client?.prefs?.read_preference(/datum/preference/toggle/weird_sounds))
...
////////////////////////// Sound vars /////////////////////////////
	...
	switch(digest_mode)
		...
		if(DM_DIGEST)
			...
			for (var/mob/living/M in contents)
				if(prob(25))
					if(M && M.client && M.client?.prefs?.read_preference(/datum/preference/toggle/weird_sounds))
	...
	if(play_sound)
		if((world.time + NORMIE_HEARCHECK) > last_hearcheck)
			...
			for(var/mob/M in hearers(VORE_SOUND_RANGE, owner))
				if(!M.client || !(M.client?.prefs?.read_preference(/datum/preference/toggle/weird_sounds)))
					continue
