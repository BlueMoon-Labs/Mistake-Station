...
/obj/item/dogborg/sleeper/proc/clean_cycle(mob/living/silicon/robot/hound)
					...
					if((world.time - NORMIE_HEARCHECK) > last_hearcheck)
						...
						for(var/mob/H in hearing_mobs)
							if(!istype(H.loc, /obj/item/dogborg/sleeper))
								H.playsound_local(source, null, 45, sound_to_use = pred_death)
							else if(H in contents)
								H.playsound_local(source, null, 65, sound_to_use = prey_death)
	...
	if(prob(50))
		if((world.time - NORMIE_HEARCHECK) > last_hearcheck)
			...
			for(var/mob/H in hearing_mobs)
				if(!istype(H.loc, /obj/item/dogborg/sleeper))
					H.playsound_local(source, null, 45, sound_to_use = pred_digest)
				else if(H in contents)
					H.playsound_local(source, null, 65, sound_to_use = prey_digest)
