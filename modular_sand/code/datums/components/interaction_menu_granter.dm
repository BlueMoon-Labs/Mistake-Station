...
/datum/component/interaction_menu_granter/ui_data(mob/user)
	...
	if(prefs)
	//Getting char prefs
		.["erp_pref"] = 			prefs.read_preference(/datum/preference/toggle/master_erp_preferences)
		.["noncon_pref"] = 			prefs.read_preference(/datum/preference/choiced/erp_yesnoask/noncon_pref)
		.["vore_pref"] = 			prefs.read_preference(/datum/preference/choiced/erp_yesnoask/vore_pref)
		.["extreme_pref"] = 		prefs.read_preference(/datum/preference/toggle/erp/extreme_pref)
		.["extreme_harm"] = 		prefs.read_preference(/datum/preference/toggle/erp/extreme_harm)
		.["unholy_pref"] =			prefs.read_preference(/datum/preference/toggle/erp/unholy_harm)

	//Getting preferences toggle
		.["verb_consent"] = 		prefs.read_preference(/datum/preference/toggle/master_erp_preferences)
		.["lewd_verb_sounds"] = 	prefs.read_preference(/datum/preference/toggle/master_erp_preferences)
		.["arousable"] = 			prefs.read_preference(/datum/preference/toggle/master_erp_preferences)
		.["genital_examine"] = 		prefs.read_preference(/datum/preference/toggle/master_erp_preferences)
		.["vore_examine"] = 		prefs.read_preference(/datum/preference/choiced/erp_yesnoask/vore_pref)
		.["medihound_sleeper"] =	prefs.read_preference(/datum/preference/choiced/erp_yesnoask/vore_pref)
		.["eating_noises"] = 		prefs.read_preference(/datum/preference/toggle/weird_sounds)
		.["digestion_noises"] =		prefs.read_preference(/datum/preference/toggle/weird_sounds)
		.["trash_forcefeed"] = 		prefs.read_preference(/datum/preference/toggle/erp/trash_forcefeed)
		.["forced_fem"] = 			prefs.read_preference(/datum/preference/toggle/erp/gender_change)
		.["forced_masc"] = 			prefs.read_preference(/datum/preference/toggle/erp/gender_change)
		.["bimbofication"] = 		prefs.read_preference(/datum/preference/toggle/erp/bimbofication)
		.["breast_enlargement"] = 	prefs.read_preference(/datum/preference/toggle/erp/breast_enlargement)
		.["penis_enlargement"] =	prefs.read_preference(/datum/preference/toggle/erp/penis_enlargement)
		.["butt_enlargement"] =		prefs.read_preference(/datum/preference/toggle/erp/butt_enlargement)
		.["belly_enlargement"] = 	prefs.read_preference(/datum/preference/toggle/erp/belly_enlargement)
		.["breast_shrinkage"] = 	prefs.read_preference(/datum/preference/toggle/erp/breast_shrinkage)
		.["penis_shrinkage"] =		prefs.read_preference(/datum/preference/toggle/erp/penis_shrinkage)
		.["butt_shrinkage"] =		prefs.read_preference(/datum/preference/toggle/erp/butt_shrinkage)
		.["belly_shrinkage"] = 		prefs.read_preference(/datum/preference/toggle/erp/belly_shrinkage)
		.["hypno"] = 				prefs.read_preference(/datum/preference/toggle/erp/hypno)
		.["never_hypno"] = 			prefs.read_preference(/datum/preference/toggle/erp/never_hypno)
		.["no_aphro"] = 			prefs.read_preference(/datum/preference/toggle/erp/aphro)
		.["no_ass_slap"] = 			prefs.read_preference(/datum/preference/toggle/erp/ass_slap)
		.["no_auto_wag"] = 			prefs.read_preference(/datum/preference/toggle/erp/auto_wag)
			...
			switch(params["char_pref"])
				if("erp_pref")
					if(prefs.erppref == value)
						...
						prefs.erppref = value
				if("noncon_pref")
					if(prefs.noncon_pref == value)
						return FALSE
					else
						prefs.noncon_pref = value
				if("vore_pref")
		...
		if("pref")
			...
			switch(params["pref"])
				...
				if("belly_enlargement")
					TOGGLE_BITFIELD(prefs.cit_toggles, BELLY_ENLARGEMENT)
				if("breast_shrinkage")
					TOGGLE_BITFIELD(prefs.cit_toggles, BREAST_SHRINKAGE)
				if("penis_shrinkage")
					TOGGLE_BITFIELD(prefs.cit_toggles, PENIS_ENLARGEMENT)
				if("butt_shrinkage")
					TOGGLE_BITFIELD(prefs.cit_toggles, BUTT_ENLARGEMENT)
