...
/datum/mood/proc/HandleThirst(mob/living/L)
	...
	switch(get_thirst(L))
		if(THIRST_LEVEL_QUENCHED to INFINITY)
			add_mood_event(null, "thirst", /datum/mood_event/quenched)
		if(THIRST_LEVEL_THIRSTY to THIRST_LEVEL_QUENCHED)
			clear_mood_event(null, "thirst")
		if(THIRST_LEVEL_PARCHED to THIRST_LEVEL_THIRSTY)
			add_mood_event(null, "thirst", /datum/mood_event/thirsty)
		if(0 to THIRST_LEVEL_PARCHED)
			add_mood_event(null, "thirst", /datum/mood_event/dehydrated)
