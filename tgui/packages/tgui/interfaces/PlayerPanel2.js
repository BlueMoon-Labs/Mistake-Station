...
import { Input, Button, Flex, Section, Tabs, Box, NoticeBox, NumberInput, Collapsible, LabeledList, Dropdown, Slider } from '../components';
...
const PhysicalActions = (props, context) => {
  ...
  const [mobScale, setMobScale] = useLocalState(
    context,
    'mobScale',
    initial_scale
  );
      ...
      <Section
        ...
        <Flex wrap="wrap" justify="space-between">
          {roles[jobbanTab].category_name === 'Antagonists' && (
            <NoticeBox width="100%" danger={antag_ban_reason ? true : false}>
              <Flex justify="space-between" align="center">
                <Flex.Item width="100%">
                  This player is {antag_ban_reason ? '' : 'not'} antagonist
                  banned
                </Flex.Item>
                <Flex.Item>
                  <Button
                    ...
                    tooltip={
                      antag_ban_reason ? 'Reason: ' + antag_ban_reason : ''
                    }
          ...
          {roles[jobbanTab].category_roles.map((role) => {
            return (
              <Flex.Item key={0} width="49%">
                <Button
                  width="100%"
                  py=".5rem"
                  mb=".5rem"
      ...
      <Section title="Teleportation">
        <Flex>
          <Button.Confirm
            ...
            onClick={() => act('bring')}
          />
          <Button width="100%" content="Orbit" onClick={() => act('orbit')} />
          <Button.Confirm
            width="100%"
            height="100%"
            icon="share"
      ...
      <Section
        ...
        <Collapsible
          ...
          <LabeledList>
            ...
            <LabeledList.Item label="Active bans">
              {active_role_ban_count}
            </LabeledList.Item>
          </LabeledList>
        </Collapsible>
...
const FunActions = (props, context) => {
  ...
  const [narrateMessage, setNarrateMessage] = useLocalState(
    context,
    'narrateMessage',
    ''
  );
  const [narrateColour, setNarrateColour] = useLocalState(
    context,
    'narrateColour',
    Object.keys(colours)[0]
  );
  const [narrateFont, setNarrateFont] = useLocalState(
    context,
    'narrateFont',
    'Verdana'
  );
  const [narrateBold, setNarrateBold] = useLocalState(
    context,
    'narrateBold',
    false
  );
  const [narrateItalic, setNarrateItalic] = useLocalState(
    context,
    'narrateItalic',
    false
  );
  const [narrateGlobal, setNarrateGlobal] = useLocalState(
    context,
    'narrateGlobal',
    false
  );
