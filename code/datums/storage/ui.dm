...
/**
  ...
  */
/datum/storage/proc/orient2hud_legacy(mob/user, maxcolumns)
	. = list()
	...
	var/columns = clamp(max_slots, 1, maxcolumns ? maxcolumns : screen_max_columns)
...
/**
  ...
  */
/datum/storage/proc/orient2hud_volumetric(mob/user, maxcolumns)
	. = list()
...
/**
  ...
  */
/datum/storage/proc/ui_show(mob/M)
	if(!M.client)
		return FALSE
	...
	LAZYOR(is_using, M)
	if(!M.client?.prefs?.read_preference(/datum/preference/toggle/no_tetris_storage) && volumetric_ui())
...
/**
  ...
  */
/datum/storage/vv_edit_var(var_name, var_value)
	var/list/old
...
/**
  ...
  */
/datum/storage/proc/on_logout(datum/source, client/C)
	ui_hide(source)

/**
  * Hides our UI from a mob
  */
/datum/storage/proc/ui_hide(mob/M)
	if(!M.client)
		return TRUE
...
/**
  ...
  */
/datum/storage/proc/volumetric_ui()
