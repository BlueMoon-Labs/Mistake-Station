...
/obj/effect/grand_rune/proc/announce_rune()
	...
	if (potency >= GRAND_RITUAL_IMMINENT_FINALE_POTENCY)
		priority_announce("Major anomalous fluctuations to local spacetime detected in: [created_area.name].", "Аномальная Тревога")
		return
	if (potency >= GRAND_RITUAL_RUNES_WARNING_POTENCY)
		priority_announce("Unusual anomalous energy fluctuations detected in: [created_area.name].", "Аномальная Тревога")
		return
...
/obj/effect/grand_rune/finale/invoke_rune(mob/living/user)
	...
	if (announce)
		priority_announce(announce, "Аномальная Тревога")
	dire_warnings_given++
	return ..()
