...
/datum/round_event/shuttle_insurance/announce(fake)
	priority_announce("Входящая подпространственная передача данных. Открыт защищенный канал связи на всех коммуникационных консолях.", "Предложение о Защите Шаттла", SSstation.announcer.get_rand_report_sound())
...
/datum/round_event/shuttle_insurance/proc/answered()
	if(EMERGENCY_AT_LEAST_DOCKED)
		priority_announce("Друзья, вы определенно опоздали с приобретением страховки. Наши агенты уже покинули ваш Сектор.",sender_override = ship_name)
		return
	if(insurance_message && insurance_message.answered == 1)
		var/datum/bank_account/station_balance = SSeconomy.get_dep_account(ACCOUNT_CAR)
		if(!station_balance?.adjust_money(-insurance_evaluation))
			priority_announce("Вы не прислали нам достаточно денег на страховку шаттла. Это, в терминах космического обывателя, считается мошенничеством. Мы оставим ваши деньги себе, мошенники!",sender_override = ship_name)
			return
		priority_announce("Спасибо за покупку Страховки для вашего Шаттла Эвакуации!",sender_override = ship_name)
